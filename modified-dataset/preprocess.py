...
for i,content in enumerate(data):
    sensor[i] = pd.read_csv("temp/"+ content, delimiter= ",")
    ...
    for j in range(0, len(sensor[i]["timestamp"])):
        ...
        print(sensor[i]["timestamp"][j].split(" ")[0].split("-")[2] + sensor[i]["timestamp"][j].split(" ")[1].split(":")[0] + sensor[i]["timestamp"][j].split(" ")[1].split(":")[1])
        exit()
