...
###1 --obs_len: Number of time-steps in input trajectories. Default is 8.
###2 --pred_len: Number of time-steps in output trajectories. Default is 8.
###3 --embedding_dim: Integer giving the dimension for the embedding layer 
####                  for input (x, y) coordinates. Default is 64.
###4 --num_layers: Number of layers in LSTM. We only support num_layers = 1.
###5 --mlp_dim: Integer giving the dimensions of the MLP. Default is 1024. 
#               We use the same mlp options across all three components of the model.
###6 --noise_dim: Integer tuple giving the dimensions of the noise added to the input of the decoder. Default is None.
###8 --noise_type: Type of noise to be added. We support two options "uniform" and "gaussian" noise. Default is "gaussian".
###9 --noise_mix_type: The added noise can either be the same across all pedestrians or we can have a different per person noise.
#                      We support two options "global" and "ped". Default value is "ped".
###10 --pooling_type: Type of pooling module to use. We support two options "pool_net" and "spool". Default is "pool_net".
###12 --pool_every_timestep: We can pool the hidden states at every time step or only once after obs_len. Default is false.
###13 --bottleneck_dim: Output dimensions of the pooled vector for Pool Net. Default is 1024.
###14 --neighborhood_size: Neighborhood size to consider in social pooling. Please refer to S-LSTM paper for details. Default is 2.
###15 --grid_size: The neighborhood is divided into grid_size x grid_size grids. Default is 8.
###16 --d_type: The discriminator can either treat each trajectory independently as described in the paper (option "local") or 
#               it can follow something very similar to the generator and pool the information across trajectories to determine 
#               if they are real/fake (option "global"). Default is "local".
